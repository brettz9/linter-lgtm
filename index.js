...
export function activate () { // Optional
  debug && console.log('activating LGTM linter...');
}
...
export function provideLinter () {
  return {
    ...
    async lint (textEditor) {
      ...
      if (!org || !name) {
        warn(`no org or name found for provider ${provider}`, repoOrigin, editorPath);
        return [];
      }
      ...
      let commitID;
      try {
          const {stdout, stderr} = await exec('git rev-parse HEAD', opts);
      ...
      let diff;
      try {
          const {stdout, stderr} = await exec('git diff --binary', opts);
      ...
      if (provider === 'g') { // Find fork parent for Github since LGTM apparently only tracks the originals
          ...
          if (forkInfo) {
              ...
              if (!org || !name) {
                warn(`no org or name found for fork with owner ${repoURL.owner} and name ${repoURL.name} provider ${provider}`, repoOrigin, editorPath);
                return [];
              }
              log('fork org', org);
              log('fork name', name);
      ...
      // 5. Get project ID:
      let projectID;
      let getProviderURL = `https://lgtm.com/api/v1.0/projects/${provider}/${org}/${name}`;
      log('getProviderURL', getProviderURL);
      try {
          let providerInfoResp = await request({
              url: getProviderURL,
              json: true,
              headers: {
                  Accept: 'application/json',
